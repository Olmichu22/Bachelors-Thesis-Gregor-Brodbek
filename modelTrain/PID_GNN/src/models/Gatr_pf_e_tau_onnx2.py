...
class ExampleWrapper(L.LightningModule):
    ...
    def forward(self, inputs1): #,inputs_scalar, energy_inputs, feature_high_level): # inputs_scalar, energy_inputs, feature_high_level):
        """Forward pass.
        ...
        Returns
        -------
        outputs : torch.Tensor with shape (*batch_)imensions, 1)
            Model prediction: a single scalar for the whole point cloud.
        """
        inputs = inputs1[:,0:3]
        inputs_scalar = inputs1[:,3].unsqueeze(1)
        energy_inputs = inputs1[:,4:6]
        feature_high_level = inputs1[:,6:9]
        inputs_BN = self.ScaledGooeyBatchNorm2_1(inputs)
        embedded_inputs_1 = embed_point(inputs_BN) 
        
        embedded_inputs_2 = embedded_inputs_1.unsqueeze(
            -2
        ) 
        scalars = torch.cat(
        ...
        embedded_outputs, scalar_outputs = self.gatr(
            embedded_inputs_2, scalars=scalars
        )  
    
        hg = torch.cat(
            (embedded_outputs.squeeze(1), scalar_outputs), dim=1
        )
       
        hg = torch.sum(hg , dim=0)
        feature_high_level1 = torch.sum(feature_high_level, dim=0).unsqueeze(0)
     
        all_features = torch.cat((feature_high_level1, hg.unsqueeze(0)), dim=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        return loss
    def obtain_high_level_features(self, g):
        ...
        features_high_level = torch.cat((g.ndata["hit_type1"].view(-1,1),g.ndata["hit_type2"].view(-1,1), g.ndata["hit_type3"].view(-1,1), g.ndata["hit_type4"].view(-1,1)), dim=1)
        return features_high_level
    ...
    def validation_step(self, batch, batch_idx):
        ...
        torch.save(
            dic,
            self.args.model_prefix + "/graphs_base_v111_4/" + str(batch_idx) + "_previous.pt",
        )
        # feature_high_level = self.obtain_high_level_features(batch_g) 
        feature_high_level = self.obtain_high_level_features(batch_g) 
        inputs1 = torch.cat((inputs,inputs_scalar,energy_inputs,feature_high_level), dim=1)
        torch.save(inputs1, str(batch_idx) +".pt")
        model_output = self(inputs1)
        print(model_output)
        labels_true = batch_g.ndata["label_true"].view(-1)[0]
